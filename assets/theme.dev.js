...
(function (scrollLock) {
    ...
    document.addEventListener('DOMContentLoaded', function() {
      function toggleDescription(e) {
        var toggle = e.target.closest('.product-description-toggle');
        if (!toggle) return;
        var container = toggle.parentElement;
        var content = container.querySelector('.product-description-content');
        if (content) {
      ...
      document.body.addEventListener('keydown', function(e) {
        if ((e.key === 'Enter' || e.key === ' ') && e.target.classList.contains('product-description-toggle')) {
          e.preventDefault();
          toggleDescription(e);
        }
      });
    ...
    */
    ...
    if (!customElements.get('header-search-form')) {
      ...
      customElements.define(
        ...
        class PredictiveSearch extends HeaderSearchForm {
          ...
          getSearchResults(searchTerm) {
            ...
            fetch(`${theme.routes.predictive_search_url}?q=${encodeURIComponent(searchTerm)}&resources[options][fields]=title,product_type,variants.title,vendor,variants.sku&section_id=predictive-search`, {signal: this.abortController.signal})
              .then((response) => {
                if (!response.ok) {
